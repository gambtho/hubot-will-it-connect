...
describe 'will-it-connect', ->
  ...
  beforeEach ->
    ...
    nock("http://willitconnect.cfapps.io")
  ...
  context 'user makes a bad request', ->
    ...
    it 'and it should reply with an error message for a request without a port',  ->
      expect(room.robot.emit.lastCall.args[1].content.title).equals("Invalid Url - please use this format: willitconnect <url:port>")
  ...
  context 'user makes a request that cannot connect', ->
    ...
    it 'and it should reply with unable to connect information',  ->
      console.log room.robot.emit.lastCall.args[1].content
      expect(room.robot.emit.lastCall.args[1].content.title).to.match(/Willitconnect error - I am unable to connect to willItConnect - Error:/)
